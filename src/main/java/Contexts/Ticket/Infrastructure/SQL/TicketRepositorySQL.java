    ...
    public void newTicket(Map<Product, Integer> ticket) {
        try {
            ...
            if (generateKey.next()) {
                ...
                Ticket newTicket = new Ticket(ticketId, java.sql.Date.valueOf(java.time.LocalDate.now())); // Crear un nuevo objeto Ticket con el ID y la fecha actual
        ...
        public List<Ticket> getAllTickets() {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    Ticket ticket = ticketMap.getOrDefault(ticketId, date);
